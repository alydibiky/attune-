...
with sync_playwright() as pw:
    ...
    check("FLUX.2 klein 4B" in ins.inner_text() and "5.29" in ins.inner_text() and "Apache-2.0" in ins.inner_text(), "the install card names the model, its size and licence")
    ...
    check([f["role"] for f in a["files"]] == ["diffusion", "llm", "vae"] and a["files"][0]["url"].endswith("/flux-2-klein-4b-Q4_0.gguf")
    ...
    check([li["width"], li["height"], li["steps"], li["pack"]] == [1024, 1024, 4, "klein-4b"], "1024×1024, 4 steps, FLUX.2 klein")
    ...
    ctx, page = new_page(br, env, errors, extra_init=MOCK + "\nwindow.__mock.img.built = false; window.__mock.img.gpuBuilt = false;")
    page.goto(env.url); page.wait_for_selector("nav", timeout=15000)
    open_studio(page)
