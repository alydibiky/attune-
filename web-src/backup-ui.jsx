...
function Summary({ s }) {
  const rows = [
    [t("Chats"), s.chats], [t("Money ledger entries"), s.ledger], [t("Cycle days logged"), s.cycleDays],
    [t("Memory items"), s.memory], [t("Reminders"), s.reminders],
  ];
  return (
    <div className="grid grid-cols-2 gap-1.5 text-[12px]">
      ...
      <div className="flex justify-between gap-2 rounded-lg bg-slate-950 border border-slate-800 px-2.5 py-1.5">
        <span className="text-slate-400">{t("Size")}</span><span className="text-slate-100 font-medium" dir="ltr">{kb(s.bytes || 0)}</span>
      </div>
    </div>
  );
}
...
export function BackupPanel({ close, flash, nativeCall, native }) {
  ...
  async function doBackup() {
    ...
    if (p) return setErr(t(p));
    setErr(""); setDone(""); setBusy(t("Locking your data…"));
    try {
      const text = await encryptBackup(collectData(), pw, { app: "5" });
      const name = backupFileName();
      if (native) {
        setBusy(t("Choose where to save it…"));
        const r = await nativeCall("saveFile", { name, mime: "application/octet-stream", text });
        setDone(t("Saved as {name}", { name: r.name || name }));
      } else {
        const a = document.createElement("a");
        a.href = URL.createObjectURL(new Blob([text], { type: "application/octet-stream" }));
        a.download = name; document.body.appendChild(a); a.click(); a.remove();
        setTimeout(() => URL.revokeObjectURL(a.href), 5000);
        setDone(t("Saved as {name}", { name }));
      }
      try { localStorage.setItem(LAST_BACKUP_KEY, String(Date.now())); } catch (e) {}
      setPw(""); setPw2("");
    } catch (e) {
      setErr(/cancel/i.test(e.message) ? t("Not saved — you closed the picker.") : e.message);
    } finally { setBusy(""); }
  }
  ...
  async function pick(e) {
    ...
    } catch (x) { setFile(null); setErr(t(x.message)); }
  }

  async function unlock() {
    if (!pw) return setErr(t("Type the backup's password."));
    setErr(""); setBusy(t("Unlocking…"));
    try { setOpened(await decryptBackup(file.text, pw)); }
    catch (x) { setErr(t(x.message)); }
    finally { setBusy(""); }
  }
  ...
  function doRestore() {
    ...
    try {
      ...
      flash && flash(t("Restored — reopening…"));
      setTimeout(() => location.reload(), 600);
      setDone(t("Restored — reopening…"));
    } catch (x) { setErr(t(x.message)); }
  }
    ...
    <div className="fixed inset-0 z-[60] bg-black/70 flex items-center justify-center p-4" onClick={close}>
      <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-5" onClick={(e) => e.stopPropagation()} data-testid="backup-panel">
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-lg font-bold text-white flex items-center gap-2"><ShieldCheck size={19} className="text-teal-300" />{t("Backup & restore")}</h2>
          <button onClick={close} className="text-slate-500" aria-label={t("Close")}><X size={20} /></button>
        </div>
        <p className="text-xs text-slate-500 mb-3">{t("Everything in Attune lives only on this phone. A backup is one file, locked with your password, that you can keep anywhere.")}</p>
        <div className="flex gap-1 bg-slate-950 border border-slate-800 rounded-xl p-1 mb-4">{tabBtn("backup", t("Back up"))}{tabBtn("restore", t("Restore"))}</div>

        {tab === "backup" ? (
          <div className="space-y-3">
            <p className="text-[11px] uppercase tracking-wider text-slate-500">{t("What will be saved")}</p>
            <Summary s={now} />
            <p className="text-[11px] text-slate-500">{since == null ? t("You have never made a backup.") : since === 0 ? t("Last backup: today.") : t("Last backup: {n} days ago.", { n: since })} {t("Models are not included — download them again from Engine.")}</p>
            <PwBox value={pw} onChange={setPw} placeholder={t("Password (8+ characters)")} show={show} testid="bk-pw" />
            <PwBox value={pw2} onChange={setPw2} placeholder={t("Same password again")} show={show} testid="bk-pw2" />
            <div className="flex items-center justify-between text-[11px]">
              <div className="flex gap-1 items-center">
                ...
                <span className="text-slate-500 ms-1">{[t("Too short"), t("Weak"), t("Good"), t("Strong")][strength]}</span>
              </div>
              <label className="text-slate-400 flex items-center gap-1"><input type="checkbox" checked={show} onChange={(e) => setShow(e.target.checked)} />{t("Show")}</label>
            </div>
            <div className="flex gap-2 items-start rounded-lg border border-amber-700/50 bg-amber-500/10 p-2.5 text-[11px] text-amber-200">
              ...
              <span>{t("If you forget this password the backup cannot be opened — by anyone. Write it down somewhere safe.")}</span>
            </div>
            <button onClick={doBackup} disabled={!!busy} data-testid="bk-save"
              ...
              {busy ? <Loader2 size={16} className="animate-spin" /> : <Lock size={16} />}{busy || t("Lock and save backup")}
            </button>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            <button onClick={() => fileRef.current && fileRef.current.click()}
              ...
              <Download size={16} />{file ? t("Choose a different file") : t("Choose backup file")}
            </button>
            {file ? (
              <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 space-y-2">
                <p className="text-[12px] text-slate-300 break-all">{file.name}</p>
                <p className="text-[11px] text-slate-500">{t("Made on {d}", { d: new Date(file.head.created).toLocaleString() })}</p>
                {file.head.summary ? <Summary s={file.head.summary} /> : null}
              </div>
            ) : null}
            {file && !opened ? (
              <>
                <PwBox value={pw} onChange={setPw} placeholder={t("Backup password")} show={show} autoFocus testid="rs-pw" />
                <button onClick={unlock} disabled={!!busy} data-testid="rs-unlock"
                  className="w-full flex items-center justify-center gap-2 bg-teal-500 disabled:opacity-60 text-slate-950 font-semibold rounded-xl py-2.5">
                  {busy ? <Loader2 size={16} className="animate-spin" /> : <Lock size={16} />}{busy || t("Unlock")}
                </button>
              </>
            ) : null}
            {opened ? (
              <div className="space-y-2">
                <p className="text-[12px] text-teal-300 flex items-center gap-1.5"><CheckCircle2 size={14} />{t("Unlocked. How should it be restored?")}</p>
                {[["replace", t("Replace everything on this phone"), t("This phone becomes exactly like the backup.")],
                  ["merge", t("Merge"), t("Keep everything on this phone and add the backup's chats, memory, reminders and cycle days. The Money ledger is only taken from the backup if this phone has none.")]].map(([id, a, b]) => (
                ...
                {!confirm ? (
                  <button onClick={() => setConfirm(true)} data-testid="rs-restore" className="w-full bg-teal-500 text-slate-950 font-semibold rounded-xl py-2.5">{t("Restore")}</button>
                ) : (
                  <div className="rounded-xl border border-rose-700/60 bg-rose-500/10 p-3 space-y-2">
                    <p className="text-[12px] text-rose-200">{mode === "replace"
                      ? t("This phone's {c} chats and {l} ledger entries will be replaced by the backup's. You can undo this once, right after.", { c: now.chats, l: now.ledger })
                      : t("The backup's items will be added to this phone. You can undo this once, right after.")}</p>
                    <div className="flex gap-2">
                      <button onClick={() => setConfirm(false)} className="flex-1 border border-slate-700 rounded-lg py-2 text-sm text-slate-300">{t("Cancel")}</button>
                      <button onClick={doRestore} data-testid="rs-confirm" className="flex-1 bg-rose-500 text-white font-semibold rounded-lg py-2 text-sm">{t("Yes, restore")}</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            {hasUndo && !opened ? (
              <button onClick={undoRestore} data-testid="rs-undo" className="w-full flex items-center justify-center gap-2 text-[12px] text-slate-400 border border-slate-800 rounded-xl py-2">
                <RefreshCw size={14} />{t("Undo the last restore")}
              </button>
            ) : null}
          </div>
        )}
...
export function backupNudge() {
  ...
  if (d == null) return { text: t("Never — do it now"), warn: true };
  if (d === 0) return { text: t("Backed up today"), warn: false };
  return { text: t("{n} days ago", { n: d }), warn: d >= 14 };
}
